...
test('Central upstream authentication is present in gateway source',async()=>{const src=await fs.readFile(new URL('../server.mjs',import.meta.url),'utf8');assert.match(src,/NEXO_UPSTREAM_SHARED_SECRET/);assert.match(src,/CENTRAL_AUTH_NOT_CONFIGURED/);assert.match(src,/x-nexo-gateway-auth/)});
