...
async function send(node,targetRole,type,payload,{targetNodeId='',correlationId=`COR-${crypto.randomUUID()}`}={}){const now=new Date(),env={schema:'NEXO_COMM_ENVELOPE_V1',schemaVersion:1,compatibilityId:COMPAT,messageId:`MSG-${crypto.randomUUID()}`,taskId:'',correlationId,sourceRole:node.role,sourceNodeId:node.nodeId,targetRole,targetNodeId,type,createdAt:now.toISOString(),expiresAt:new Date(now.getTime()+600000).toISOString(),payload,payloadHash:sha(payload)};return hub.send({credential:node.credential,envelope:env,signature:sign(node.key,env)});}
async function poll(node){const p={schema:'NEXO_HUB_POLL_V1',nodeId:node.nodeId,role:node.role,issuedAt:new Date().toISOString(),nonce:crypto.randomBytes(12).toString('base64url'),limit:20,afterSequence:0};return hub.poll({credential:node.credential,poll:p,signature:sign(node.key,p)});}
async function ack(node,ids){const a={schema:'NEXO_HUB_ACK_V1',nodeId:node.nodeId,role:node.role,issuedAt:new Date().toISOString(),nonce:crypto.randomBytes(12).toString('base64url'),messageIds:ids};return hub.ack({credential:node.credential,ack:a,signature:sign(node.key,a)});}

test('private CLIENTE reply is visible only to target node',async()=>{const c1=enroll('CLIENTE','CLIENT-A'),c2=enroll('CLIENTE','CLIENT-B'),nexa=enroll('NEXA','NEXA-1');const sent=await send(nexa,'CLIENTE','SUPPORT_MESSAGE',{answer:'ok'},{targetNodeId:c1.nodeId});assert.equal(sent.targetNodeId,c1.nodeId);assert.equal((await poll(c2)).messages.length,0);const p1=await poll(c1);assert.equal(p1.messages.length,1);assert.equal(p1.messages[0].envelope.targetNodeId,c1.nodeId);assert.equal((await ack(c2,[p1.messages[0].envelope.messageId])).acked,0);assert.equal((await ack(c1,[p1.messages[0].envelope.messageId])).acked,1);});
